...
def solve_with_nsga2(
    ...
    evolutionary algorithm that uses non-dominated sorting and crowding distance to
    maintain a diverse set of Pareto-optimal solutions.

    Args:
        sims_instance: The SIMS problem instance to solve
        objectives: List of exactly 4 objectives to optimize. Valid values:
            - "min_cost": Minimize total acquisition cost
            - "cloud_coverage": Minimize total cloudy area
            - "min_resolution": Minimize total resolution (better resolution = lower value)
            - "min_max_incidence_angle": Minimize maximum incidence angle
        timeout: Maximum runtime as timedelta
    ...
    Raises:
        ValueError: If the number of objectives is not exactly 4, or if invalid objectives are specified.

    Examples:
        # Basic 4D optimization
        result = solve_with_nsga2(problem)
        solutions = result.final_solutions
        ...
        # Without trace
        result = solve_with_nsga2(problem, trace=False)
    """
    ...
...
def solve_with_moead(
    ...
    MOEA/D (Multi-Objective Evolutionary Algorithm based on Decomposition) decomposes a
    multi-objective optimization problem into a set of scalar subproblems using weight vectors
    and solves them cooperatively. The actual population size is determined by the simplex-lattice
    design and may differ from `population_size` (which is used as a target).
    ...
    Args:
        ...
        objectives: List of exactly 4 objectives to optimize. Valid values:
            ...
            - "min_max_incidence_angle": Minimize maximum incidence angle
        timeout: Maximum runtime as timedelta
        population_size: Target population size (actual size determined by simplex-lattice design)
        max_generations: Maximum number of generations to run
        seed: Random seed for reproducibility
        trace: Whether to generate optimization trace archive (default True)
        objective_bounds: Optional list of [min, max] bounds for each objective, used for trace generation.
    ...
    Examples:
        # Basic 4D optimization
        result = solve_with_moead(problem)
        solutions = result.final_solutions

        # With custom parameters
        result = solve_with_moead(
            problem,
            population_size=300,
            max_generations=10000,
            timeout=timedelta(seconds=60),
        ...
        # Without trace
        result = solve_with_moead(problem, trace=False)
    """
    ...
