...
class ParetoFront:
    ...
    def dominates(self, x, y):
        ...
        return all(
            [
                self.compare(int(obj1), int(obj2), m)
                for (obj1, obj2, m) in zip(x["objs"], y["objs"], x["minimize_objs"])
            ]
        )
    ...
    def front_constraint_mzn(self):
        ...
        for f in self.front:
            cons.append(
                "(" + self.not_dominated_constraint_mzn(self.solutions[f]) + ")"
            )
        return " /\\ ".join(cons)
    ...
    def to_str(self):
        ...
        return (
            "{" + ",".join([str(self.solutions[f]["objs"]) for f in self.front]) + "}"
        )
