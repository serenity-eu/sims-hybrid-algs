...
use pls::{
    PlsOptimizations, SetCoverProblem,
    concurrent_pls::decomposition::{
        auto_select_h, belongs_to_region, build_regions, das_dennis_weight_vectors,
    },
...
fn main() {
    ...
    for path in &paths {
        ...
        for j in 0..D {
            ...
            println!(
                "    {:>15} {:>12} {:>12} {:>12} {:>12.1} {:>12.1} {:>12}",
                OBJ_NAMES[j], actual_min[j], actual_max[j], pf_range, means[j], stds[j], max_obj[j]
            );
        }
        ...
        let pf_bounds: [(f64, f64); D] =
            std::array::from_fn(|j| (actual_min[j] as f64, actual_max[j] as f64));
        analyze_decomposition(
            &obj_values,
            &pf_bounds,
            &ideal_current,
        ...
        let padded_bounds: [(f64, f64); D] = std::array::from_fn(|j| {
            ...
            (
                (actual_min[j] as f64 - margin).max(0.0),
                actual_max[j] as f64 + margin,
            )
        });
        analyze_decomposition(
            &obj_values,
...
fn analyze_per_image_data(problem: &Problem) {
    ...
    if let pls::objectives::ObjectiveState::TotalCost {
        costs, max_value, ..
    } = &obj_states[0]
    {
        ...
        println!(
            "    {:>15} {:>12} {:>12} {:>12} (sum=max_value)",
            "img cost", min_c, max_c, max_value
        );
    }

    // Resolution: per-image resolutions
    if let pls::objectives::ObjectiveState::MinResolution {
        resolutions,
        max_value,
        ..
    } = &obj_states[2]
    {
        ...
        let mean_r: f64 =
            resolutions.iter().map(|&r| r as f64).sum::<f64>() / resolutions.len() as f64;
        println!(
            "    {:>15} {:>12} {:>12} {:>12.0} (mean), max_value={}",
            "img resolution", min_r, max_r, mean_r, max_value
        );
    }

    // Incidence angle: per-image angles
    if let pls::objectives::ObjectiveState::MaxIncidenceAngle {
        incidence_angles,
        max_value,
        ..
    } = &obj_states[3]
    {
        ...
        let mean_a: f64 =
            incidence_angles.iter().map(|&a| a as f64).sum::<f64>() / incidence_angles.len() as f64;
        println!(
            "    {:>15} {:>12} {:>12} {:>12.0} (mean), max_value={}",
    ...
    // Universe & areas
    if let pls::objectives::ObjectiveState::CloudyArea {
        areas, max_value, ..
    } = &obj_states[1]
    {
        ...
        println!(
            "    {:>15} {:>12} {:>12} {:>12.0} (mean), max_value={} (sum)",
...
fn analyze_decomposition(
    ...
    for objs in obj_values {
        ...
        let scores: Vec<f64> = regions
            .iter()
            .map(|r| r.score(objs, ideal, bounds))
            .collect();
        let best_idx = scores
            .iter()
            .enumerate()
