...
pub trait ImageSet<const D: usize>: Sized {
    /// Get iterator over selected image indices
    fn selected_images(&self) -> impl Iterator<Item = usize>;

    /// Get iterator over unselected image indices
...
/// Generic over problem type P to support both Problem<T, D> and `ProblemBitset`<D>
pub trait SIMSCore<P, const D: usize>:
    ...
    P: SetCoverProblem<D>,
{
    ...
    fn calculate_objective(&self, obj_index: usize, problem: &P) -> u64 {
    ...
    fn calculate_objective_delta(&self, obj_index: usize, image_index: usize, problem: &P) -> i64 {
    ...
    fn recalculate_objectives(&mut self, problem: &P) {
        for i in 0..D {
...
/// Generic over problem type P to support both Problem<T, D> and `ProblemBitset`<D>
pub trait SIMSModifiable<P, const D: usize>: SIMSCore<P, D>
where
    P: SetCoverProblem<D>,
{
    ...
    /// Generate neighborhood solutions lazily via iterator
...
/// Generic over problem type P to support both Problem<T, D> and `ProblemBitset`<D>
pub trait EncodedSolution<P, const D: usize>: SIMSCore<P, D> + SIMSModifiable<P, D>
where
    P: SetCoverProblem<D>,
{
...
/// Generic over problem type P to support both Problem<T, D> and `ProblemBitset`<D>
pub trait MergeableWithResidual<P, const D: usize>: EncodedSolution<P, D>
where
    P: SetCoverProblem<D>,
{
