...
#[must_use]
/// Computes the weighted sum of 2D objectives using the provided weights.
///
/// # Panics
///
/// Panics if `D` is not 2.
pub fn weighted_sum_2d<const D: usize>(
    objectives: &pareto::Objectives<D>,
    weights: (f32, f32),
...
/// Applies a delta to 2D objectives.
///
/// # Panics
///
/// Panics if `D` is not 2.
...
/// Converts 2D objectives to a tuple of (i32, i32).
///
/// # Panics
///
/// Panics if `D` is not 2.
...
/// Creates a new 2D objectives array from the given cost and cloudy area.
///
/// # Panics
///
/// Panics if `D` is not 2.
